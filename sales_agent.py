...
if uploaded_file:
    ...
    snack_df['date'] = pd.to_datetime(snack_df['date'])  # ✅ 日期列转换为 datetime 类型
    ...
    selected_streamer = st.selectbox("Select a streamer to analyze:", sorted(snack_df['streamer_id'].unique(), key=lambda x: int(x[1:])))
    filtered_df = snack_df[snack_df['streamer_id'] == selected_streamer]

    # 🎯 XGBoost 模拟模块
    st.subheader("🔮 Predict Sales Using XGBoost (Simulated)")
    ...
    st.success("✅ XGBoost-like model trained on post-sale data")
